...
As reflected in the newly added entries of Panel~D, multimodal synergy extends beyond point regression into anomaly detection and instruction steering. VLM4TS~\cite{Li2026VLM4TS} demonstrates that leveraging a frozen Vision-Language Model to visually inspect candidate anomalies proposed by a lightweight ViT screening stage lifts the detection F1-max from 0.653 to 0.814 (a 24.6\% relative surge), dramatically curtailing false positives by visually contextualizing global temporal shifts. Concurrently, ChronoSteer~\cite{Wang2025ChronoSteer} demonstrates that synthetic paired cross-modal alignment data enables a decoupled language agent to issue revision instructions over frozen foundation forecasters, slashing zero-shot normalized MSE from 0.418 to 0.310 (a 25.8\% improvement on MTSFBench-300).

\subsection{Reproducible Demonstration \& Open Benchmark Tutorial}
To foster reproducible research and empirical transparency, the repository provides an end-to-end interactive tutorial suite in \texttt{examples/}:
\begin{itemize}
    \item \textbf{Standalone Python Script (\texttt{examples/demo\_multimodal\_forecasting.py}):} Simulates a realistic Time-MMD electric grid scenario wherein an unexpected extreme meteorological alert (Winter Storm Elliott) triggers severe heating demand surges and generation curtailment. It evaluates a unimodal autoregressive baseline against a multimodal text-conditioned forecaster, visualizing the resulting 90.4\% MSE reduction in \texttt{examples/forecast\_comparison.png}.
    \item \textbf{Interactive Jupyter Notebook (\texttt{examples/demo\_multimodal\_forecasting.ipynb}):} Enables researchers to experiment interactively with lookback lengths, textual prompt variations, and cross-attention weighting parameters without requiring GPU hardware.
\end{itemize}
